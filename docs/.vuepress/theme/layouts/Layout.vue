<template>
  <a-locale-provider :locale="zh_CN">
    <div id="VuePress-theme-yur">
      <Curtain v-show="getCurtain" />
      <Header :current-page="currentPage" />
      <Content :current-page="currentPage" :current-post="currentPost" />
      <Footer />
    </div>
  </a-locale-provider>
</template>
...
export default {
  ...
  beforeCreate () {
    for (const post of this.$posts) {
      if (!Object.prototype.hasOwnProperty.call(post.frontmatter, 'banner')) {
        // <img src="https://api.ooopn.com/image/infinity/api.php"/>
        post.frontmatter.banner = require('../media/images/banner.png')
      }
    }
  ...
  methods: {
    initConfig () {
      ...
      if (comment && this.handleComment()) {
        if (comment === 'valine' && isPro() && Object.prototype.hasOwnProperty.call(this.$themeConfig, 'valine')) {
          this.vl = true
        }
      }
    },
