<template>
  <div id="yur-timeline">
    <a-timeline v-if="this.$posts.length" class="timeline" mode="alternate">
      <a-timeline-item v-for="(post, index) in this.$posts" :key="index">
        <router-link :to="post.path" :style="{color:'inherit'}">
          <div v-if="index % 2 === 0">
            <a-tag color="blue">{{ parseDate(post.frontmatter.date) }}</a-tag>
            {{ post.title }}
            <hr />
            <a-tag color="#2db7f5" v-for="tag in post.frontmatter.tags" :key="tag">{{tag}}</a-tag>
          </div>
          <div v-else>
            {{ post.title }}
